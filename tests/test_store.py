...
class TestSQLiteTrackStore:
    ...
    async def test_write_and_query_tracks(self, store: SQLiteStore) -> None:
        tracks = [
            Track(track_id=1, detection=Detection(bbox=(100, 100, 200, 200), confidence=0.9, class_name="person")),
            Track(track_id=2, detection=Detection(bbox=(300, 300, 400, 400), confidence=0.8, class_name="forklift")),
        ]
...
class TestSQLiteEventStore:
    ...
    async def test_log_and_query_events(self, store: SQLiteStore) -> None:
        events = [Event(timestamp=datetime.now(tz=timezone.utc), event_type="entry",
            zone_name="Loading Dock", track_id=1, object_class="person",
            metadata={"direction": "inward"}, confidence=0.9, frame_snapshot=b"", bbox=(100, 100, 200, 200))]
    ...
    async def test_query_events_by_type(self, store: SQLiteStore) -> None:
        events = [
            Event(timestamp=datetime.now(tz=timezone.utc), event_type="entry", zone_name="Dock",
                track_id=1, object_class="person", metadata={}, confidence=0.9, frame_snapshot=b"", bbox=(100, 100, 200, 200)),
            Event(timestamp=datetime.now(tz=timezone.utc), event_type="dwell_exceeded", zone_name="Dock",
                track_id=1, object_class="person", metadata={"dwell_seconds": 400}, confidence=0.9, frame_snapshot=b"", bbox=(100, 100, 200, 200)),
        ]
        await store.log_events(events)
