...
class SQLiteStore:
    ...
    async def write_tracks(self, tracks: list[Track], meta: FrameMeta, zone_name: str | None = None) -> None:
        async with self._session_factory() as session:
            for track in tracks:
                ...
                row = TrackRow(
                    timestamp=meta.timestamp, track_id=track.track_id,
    ...
    async def query_tracks(self, source_id: str | None = None, zone_name: str | None = None,
                           since: str | None = None, until: str | None = None, limit: int = 1000) -> list[dict[str, object]]:
        async with self._session_factory() as session:
    ...
    async def query_events(self, zone_name: str | None = None, event_type: str | None = None,
                           since: str | None = None, until: str | None = None, limit: int = 1000) -> list[dict[str, object]]:
        async with self._session_factory() as session:
            ...
            return [{"id": r.id, "timestamp": r.timestamp.isoformat() if r.timestamp else None,
                      ...
                      "confidence": r.confidence, "snapshot_path": r.snapshot_path} for r in rows]
