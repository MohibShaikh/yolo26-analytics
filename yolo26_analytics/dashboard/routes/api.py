...
async def get_zones(request: Request) -> list[dict[str, Any]]:
    ...
    if za is None:
        return []
    return [{"name": z.name, "track_classes": z.track_classes, "cooldown": z.cooldown} for z in za.zones]

@router.get("/events")
async def get_events(request: Request, zone_name: str | None = None, event_type: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
...
@router.get("/tracks")
async def get_tracks(request: Request, zone_name: str | None = None, source_id: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
    store = request.app.state.store
    if store is None:
        return []
