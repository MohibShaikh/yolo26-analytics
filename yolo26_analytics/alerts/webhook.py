...
class WebhookAlert:
    ...
    async def send(self, event: Event) -> None:
        payload = {"timestamp": event.timestamp.isoformat(), "event_type": event.event_type,
            "zone_name": event.zone_name, "track_id": event.track_id,
            "object_class": event.object_class, "metadata": event.metadata,
            "confidence": event.confidence, "bbox": list(event.bbox)}
