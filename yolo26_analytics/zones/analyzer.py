...
class ZoneAnalyzer:
    def __init__(self, zone_configs: list[ZoneConfig]) -> None:
        ...
        for zc in zone_configs:
            ...
            zone = Zone(name=zc.name, polygon=polygon_tuples, track_classes=zc.track_classes, cooldown=zc.cooldown)
            self._zones.append(zone)
            for rule in zc.analytics:
                match rule.type:
                    case "count": self._counters[zc.name] = ZoneCounting()
                    case "entry_exit": self._entry_exit[zc.name] = EntryExitDetector()
                    case "dwell": self._dwell[zc.name] = DwellTracker(alert_threshold=rule.alert_threshold or 300)
                    case "throughput": self._throughput[zc.name] = ThroughputTracker(interval=rule.interval or 3600)
        ...
        for zone in self._zones:
            ...
            if zone.name in self._entry_exit:
                for ee in self._entry_exit[zone.name].update(zone, tracks):
                    event = self._make_event(event_type=ee["type"], zone_name=zone.name,
                        track_id=ee["track_id"], object_class=ee["class_name"],
                        metadata=ee, now=now, cooldown=zone.cooldown)
                    if event: events.append(event)
            if zone.name in self._dwell:
                for de in self._dwell[zone.name].update(zone, tracks, now):
                    event = self._make_event(event_type="dwell_exceeded", zone_name=zone.name,
                        track_id=de["track_id"], object_class=de["class_name"],
                        metadata=de, now=now, cooldown=zone.cooldown)
                    if event: events.append(event)
    ...
    def _make_event(self, event_type: str, zone_name: str, track_id: int, object_class: str,
                    metadata: dict[str, Any], now: datetime, cooldown: int) -> Event | None:
        key = (track_id, zone_name, event_type)
        last = self._cooldowns.get(key)
        if last is not None and (now - last).total_seconds() < cooldown:
            return None
        self._cooldowns[key] = now
        return Event(timestamp=now, event_type=event_type, zone_name=zone_name,
            track_id=track_id, object_class=object_class, metadata=metadata,
            confidence=0.0, frame_snapshot=b"", bbox=(0, 0, 0, 0))
